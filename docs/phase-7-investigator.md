...
For an [external onboarding profile](onboarding-external-cluster.md), the agent collects evidence only for the scoped incident workload. Optional Kubernetes OOM and image-pull verifiers can persist a corroborated `ROOT_CAUSE_FOUND` report; other cases remain `INSUFFICIENT_EVIDENCE`. It does not call an LLM or request remediation.
