...
For an allowlisted `incidentpilot-demo` incident younger than 24 hours, the collector records Deployment, Service, EndpointSlice, pod, and event observations plus a bounded metric range, log range, and trace search. Error incidents start with `error_rate` and the fixed error-trace filter; `DemoCheckoutLatency` starts with the fixed `success_latency_avg` expression and slow-success trace filter. This prevents the two-minute lookback from selecting an unrelated healthy trace while still keeping arbitrary TraceQL outside the caller and model boundary. Orders-api collection also reads its single accepted ConfigMap field and, when a collected pod is in `CrashLoopBackOff` or has a repeatedly terminated `Error` container, a bounded tail from that exact pod. If a trace is found, the collector stores that trace. When Phase 8 change sources are configured, the initial collection additionally records Argo CD application state and nearby deployment history, then reads commit metadata and a bounded diff for the newest revision in that history. Targeted downstream collections do not repeat change-source reads. Each record has a source, tool, collection time, parameters, resource reference, normalized deterministic summary, and retained structured payload. Timed evidence retains the exact window. The collector uses `started_at - 2 minutes` through resolution/current time, caps it at one hour, and rejects stale incidents rather than substituting unrelated current data.

A collection persists atomically with a hard maximum of 16 records. The largest current plan is 15 records (orders evidence, one observed crash-pod log, one trace, and four optional change-intelligence records), leaving one slot without making the store unbounded.
